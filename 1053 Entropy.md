...
struct Node{
    ...
    explicit Node(char c, int occur){
        this->letter = c;
        this->occur = occur;
        this->left = nullptr;
        this->right = nullptr;
...
Node* createHuffmanTree(priority_queue<Node*, vector<Node*>, cmp> &Q){
    Node *root = nullptr;
    while(!Q.empty()){
        Node *a = Q.top();
        Q.pop();
...
void dfs(int step, Node *root){
    auto it = table.find(root->letter);
    if(it!=table.end())
        it->second = step;
    if(root->left)
...
int main(){
    while(true){
        ...
        destroy(root);
        printf("%d %d %.1f\n", s.size()*8, res, s.size()*8.f/(float)res);
    }
    return 0;
}
```
