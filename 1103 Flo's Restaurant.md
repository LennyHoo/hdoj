...
int main(){
    while(true){
        ...
        while(cin>>s&&s!="#"){
            ...
            int guest;
            cin >> guest;
            queue<int> &tables = Q[(guest-1)/2];
            int tableNum = n[(guest-1)/2];
            if(tables.size()<tableNum){
                tables.push(time);
                res += guest;
            }
            else{
                int earliestTable = tables.front();
                if(time>=earliestTable){
                    tables.pop();
                    if(time>=earliestTable+30)
                        tables.push(time);
                    else
                        tables.push(earliestTable+30);
                    res += guest;
                }
            }
        }
